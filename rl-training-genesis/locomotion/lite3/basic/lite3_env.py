...
URDF_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "../models/urdf/Lite3.urdf")
...
class Lite3Env:
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer=False):
        ...
        # foot contact detection via shank links (Genesis collapses fixed ankle joints)
        self.feet_link_indices = [
            self.robot.get_link("FL_SHANK").idx_local,
            self.robot.get_link("FR_SHANK").idx_local,
            self.robot.get_link("HL_SHANK").idx_local,
            self.robot.get_link("HR_SHANK").idx_local,
        ]
        ...
        self.last_feet_contact = torch.ones((self.num_envs, 4), dtype=gs.tc_bool, device=gs.device)
        ...
        self.first_contact = torch.zeros((self.num_envs, 4), dtype=gs.tc_bool, device=gs.device)
        self.air_time_for_reward = torch.zeros((self.num_envs, 4), dtype=gs.tc_float, device=gs.device)
    ...
    def step(self, actions):
        ...
        feet_contact = torch.stack(
            [contact_forces[:, idx, :].norm(dim=-1) > 1.0 for idx in self.feet_link_indices], dim=1
        )
        self.first_contact = (self.feet_air_time > 0.0) * feet_contact
        self.air_time_for_reward = self.feet_air_time.clone()
        
        self.feet_air_time += self.dt
        self.feet_air_time *= ~feet_contact
        self.last_feet_contact = feet_contact
    ...
    def _reset_idx(self, envs_idx=None):
        ...
        if envs_idx is None:
            ...
            self.first_contact.masked_fill_(envs_idx[:, None], False)
            self.air_time_for_reward.masked_fill_(envs_idx[:, None], 0.0)
    ...
    def _update_observation(self):
        ...
        if "clip_observations" in self.env_cfg:
            self.obs_buf.clamp_(-self.env_cfg["clip_observations"], self.env_cfg["clip_observations"])
...
# reward each foot for spending time in the air, only on touchdown
        rew = torch.sum(
            torch.clamp(self.air_time_for_reward - 0.1, min=0.0) * self.first_contact.float(),
            dim=1,
        )
